...
# Claude Symphony Workflow

This template keeps the official OpenAI Symphony `codex.command` block because the current Elixir runner launches a Codex app-server. Claude-native Symphony adapters may read the `claude.command` extension block and should preserve the same status, workpad, and validation protocol.
...
5. Keep all ticket progress in one persistent Linear comment titled `## Claude Workpad`.

## Prerequisite

Linear access must be available through a configured Linear MCP server or an injected `linear_graphql` tool. If neither is available, create or update `## Claude Workpad` with the missing tool/auth blocker and stop.

## Default Posture

- Start by determining the ticket's current status, then follow the matching status flow.
- Start every active task by opening or creating the single `## Claude Workpad` comment.
...
- `Todo`: queued; immediately move to `In Progress`, then create or update `## Claude Workpad`.
...
1. Find or create a single active Linear comment titled `## Claude Workpad`.
...
6. Create a fresh `## Claude Workpad` if the previous workpad is no longer accurate.
