...
本项目把 Claude 项目中常见的规范文件、角色分工、OpenSpec change 流程、TDD/SMART/MVP 原则、Symphony-ready ticket 编排、docs 文档分类和 Git/PR 收口要求整理为一套可直接复用的目录模板。你可以把它作为新项目的起点，也可以复制其中的规范文件到已有项目中逐步落地。
...
5. 需要一个可开源、可复制、可二次定制的 Claude Agent 模板项目的用户。
...
5. 如何在 Claude 工作流中持续执行 MVP、TDD、SMART 规范。
...
5. `.claude/skills/`：Claude 可复用工作流目录，当前承载 OpenSpec 与 harness 相关流程。
...
6. 在已有 Claude 项目中使用时，可以直接复制 `CLAUDE.md`、`CLAUDE.local.md`、`WORKFLOW.md`、`.claude/`、`docs/` 和 `openspec/`。
...
1. `CLAUDE.md` 存在，并包含 MVP、TDD、SMART、OpenSpec change 提交收口、角色协作和交付输出要求。
...
4. `.claude/skills/` 中存在 `harness-local-server`、`harness-playwright-evidence` 与 `harness-linear-loop`，用于补齐本地启动、浏览器证据和 Linear 闭环。
5. `examples/symphony-harness-smoke/` 存在，并能演示 local server、Playwright evidence 与 Workpad handoff。
6. `.claude/agents/` 中存在 `pm`、`explorer`、`builder`、`tester`、`reporter` 五类角色。
