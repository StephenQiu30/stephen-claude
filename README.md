...
12. `scripts/vendor-superpowers-skills.sh`：从 obra/superpowers 更新 `.claude/skills/`（勿用 `npx skills add`）。
...
4. `.claude/skills/` 中存在 `harness-local-server`、`harness-playwright-evidence`、`harness-linear-loop` 和 `debug`，用于补齐本地启动、浏览器证据、Linear 闭环和 Symphony 排障。
