...
一个面向 Claude 的开源 Agent 协作规范模板，用于帮助个人或团队快速建立稳定、可复用、可验收的 AI 协作工作流。

本项目把 Claude 项目中常见的规范文件、角色分工、TDD/SMART/MVP 原则、Symphony-ready ticket 编排和 Git/PR 收口要求整理为一套可直接复用的目录模板。你可以把它作为新项目的起点，也可以复制其中的规范文件到已有项目中逐步落地。

## 项目地址

GitHub: <https://github.com/StephenQiu30/stephen-cladue.git>

## 适用人群

1. 希望为 Claude 项目建立统一协作规则的开发者。
2. 希望把 AI Agent 分工固化为可维护文件结构的团队。
3. 希望将 TDD、SMART、MVP 原则写入日常 AI 协作流程的工程团队。
4. 需要一个可开源、可复制、可二次定制的 Claude Agent 模板项目的用户。

## 目录定位

`claude/` 是独立项目，不依赖同级 `codex/` 目录即可单独复制、维护和开源发布。它面向 Claude 使用场景，重点解决以下问题：

1. Claude 在项目中应该读取哪些规范文件。
2. Claude 全局规范与项目局部规范如何区分。
3. Claude 多角色协作时 PM、Explorer、Builder、Tester、Reporter 如何分工。
4. 如何在 Claude 工作流中持续执行 MVP、TDD、SMART 规范。
...
1. `CLAUDE.md`：Claude 侧长期稳定的全局协作规范。
2. `CLAUDE.local.md`：当前项目中的局部规范配置，用于和全局规则区分。
3. `WORKFLOW.md`：OpenAI Symphony 风格的 Linear ticket 调度契约与 per-ticket Agent SOP。
4. `.claude/agents/`：Claude 角色定义目录。
5. `.claude/skills/`：Claude 可复用工作流目录，含 Harness、Superpowers（TDD/计划/验证）、Linear、debug 和 Git 收口；总入口为 `harness-quality-gate`。技能**只**落在 `.claude/skills/`，不使用 `.agents/`。
...
11. `scripts/vendor-superpowers-skills.sh`：从 obra/superpowers 更新 `.claude/skills/`（勿用 `npx skills add`）。
...
1. 克隆项目：`git clone https://github.com/StephenQiu30/stephen-cladue.git`。
2. 优先阅读 `CLAUDE.md` 理解全局规则，再按需修改 `CLAUDE.local.md`。
3. 接入 Symphony 或兼容 runner 时，复制并调整 `WORKFLOW.md` 中的 `project_slug`、workspace root、hooks 和 `SYMPHONY_REPO_URL`。
4. 自定义角色时修改 `.claude/agents/` 下的角色文件。
5. 在已有 Claude 项目中使用时，可以直接复制 `CLAUDE.md`、`CLAUDE.local.md`、`WORKFLOW.md`、`.claude/` 和 `docs/` 骨架目录。
...
3. `.claude/skills/openspec-*` 是通过官方 CLI 安装的 OpenSpec workflow skills。
...
1. `CLAUDE.md` 存在，并包含 MVP、TDD、SMART、角色协作、Git/PR 收口和交付输出要求。
2. `CLAUDE.local.md` 存在，并说明它是项目局部规范配置文件。
3. `WORKFLOW.md` 存在，并包含 `tracker.kind: linear`、`project_slug`、`## Claude Workpad` 和 `Human Review` 编排规则。
4. `.claude/skills/` 中存在 `agent-browser`、`harness-local-server`、`harness-playwright-evidence`、`harness-linear-loop` 和 `debug`，用于补齐浏览器自动化、本地启动、浏览器证据、Linear 闭环和 Symphony 排障。
5. `.claude/agents/` 中存在 `pm`、`explorer`、`builder`、`tester`、`reporter` 五类角色。
...
1. Claude 侧文件命名保持 `CLAUDE.md` 与 `CLAUDE.local.md`。
2. 全局稳定规则写入 `CLAUDE.md`，项目局部规则写入 `CLAUDE.local.md`。
3. Symphony 调度配置写入 `WORKFLOW.md`，不要把 project slug、workspace root 和 runner hooks 混入 `CLAUDE.md`。
4. 角色职责写入 `.claude/agents/`，不要混入全局规则文件。
