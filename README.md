...
本项目把 Claude 项目中常见的规范文件、角色分工、OpenSpec change 流程、TDD/SMART/MVP 原则、docs 文档分类和 Git/PR 收口要求整理为一套可直接复用的目录模板。你可以把它作为新项目的起点，也可以复制其中的规范文件到已有项目中逐步落地。
...
3. `.claude/agents/`：Claude 角色定义目录。
4. `.claude/skills/`：Claude 可复用工作流目录，当前主要承载 OpenSpec 相关流程。
...
3. 自定义角色时修改 `.claude/agents/` 下的角色文件。
4. 新增文档时遵循 `docs/` 分类规则，不同类型文档写入不同子目录。
5. 在已有 Claude 项目中使用时，可以直接复制 `CLAUDE.md`、`CLAUDE.local.md`、`.claude/`、`docs/` 和 `openspec/`。
...
3. `.claude/agents/` 中存在 `pm`、`explorer`、`builder`、`tester`、`reporter` 五类角色。
...
3. 角色职责写入 `.claude/agents/`，不要混入全局规则文件。
