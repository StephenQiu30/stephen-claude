...
purpose: "规划如何将 Symphony-ready ticket-level 编排规则落入 Claude 模板的 CLAUDE.md 与校验脚本。"
...
outputs:
  - "CLAUDE.md 新增 Symphony-ready 编排规范"
  - "scripts/validate-repository.sh 新增规范文本门禁"
triggers:
  - "需要将 Symphony-ready 编排规范落入 Claude Agent 模板"
downstream:
  - "CLAUDE.md"
  - "未来 WORKFLOW.md 模板"
---
...
**Goal:** 将 OpenAI Symphony 推荐的 ticket-level 编排方式落入 Claude 模板，使 `CLAUDE.md` 明确 Linear ticket、Workpad、Harness、Human Review、Rework 和 Blocked 规则。

**Architecture:** 先用 `scripts/validate-repository.sh` 增加文本门禁制造红灯，再更新 `CLAUDE.md` 让门禁变绿。`CLAUDE.md` 承载长期行为准则，未来项目级 `WORKFLOW.md` 只负责 Linear project、workspace、hooks 和 agent command 等实例配置。
...
- Modify: `CLAUDE.md`
  Responsibility: 记录 Claude 侧长期稳定的 Symphony-ready ticket-level 执行规范。
- Validate: `npm test`, `git diff --check`, `rg`.

---

### Task 1: Add Failing Claude Validation Gate
...
Insert these checks after the existing `grep -q "npm test" CLAUDE.md` line:
...
Expected: command exits with status `1` because `CLAUDE.md` does not yet contain `## Symphony-ready 编排原则`.
...
git commit -m "test: 增加 Claude Symphony-ready 编排门禁"
```
...
### Task 2: Implement Claude Symphony-ready Sections
...
```bash
rg -n "Symphony-ready 编排原则|Linear Ticket 状态机|Claude Workpad|Harness 能力要求|Human Review 门禁|Rework 与 Blocked" CLAUDE.md
```
...
```bash
git add CLAUDE.md
git commit -m "docs: 优化 Claude Symphony-ready 编排规范"
```

Expected: commit succeeds with only `CLAUDE.md` staged.

---

### Task 3: Final Claude Repository Verification
...
```text
docs: 优化 Claude Symphony-ready 编排规范
test: 增加 Claude Symphony-ready 编排门禁
docs: 规划 Claude Symphony-ready 编排实施
```
...
1. Spec coverage: this plan covers the approved design items for `CLAUDE.md`, Linear ticket state flow, `## Claude Workpad`, harness requirements, Human Review gates, Rework and Blocked handling, plus CI-enforced text gates.
2. Placeholder scan: this plan contains no placeholder markers or deferred implementation language.
3. Type consistency: all section names used in validation commands match the sections inserted into `CLAUDE.md`.
