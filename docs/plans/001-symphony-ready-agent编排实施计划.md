...
**Goal:** 将 OpenAI Symphony 推荐的 ticket-level 编排方式落入 Claude 模板，使 `CLAUDE.md` 明确长期规则，并用根目录 `WORKFLOW.md` 承接 Linear ticket 调度契约。

**Architecture:** 先用 `scripts/validate-repository.sh` 增加文本门禁制造红灯，再更新 `CLAUDE.md` 让门禁变绿。`CLAUDE.md` 承载长期行为准则，根目录 `WORKFLOW.md` 负责 Linear project、workspace、hooks 和 agent command 等实例配置。
