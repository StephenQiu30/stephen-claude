...
6. Symphony / `WORKFLOW.md` 任务必须先走 `.claude/skills/harness-quality-gate/SKILL.md`，并按阶段调用 [obra/superpowers](https://github.com/obra/superpowers) skills（`using-superpowers`、`writing-plans`、`test-driven-development`、`executing-plans`、`verification-before-completion` 等）。
7. 需要补齐本地启动和健康检查时，优先使用 `.claude/skills/harness-local-server/SKILL.md`。
8. UI 或前端任务需要端到端证据时，优先使用 `.claude/skills/harness-playwright-evidence/SKILL.md`。
9. 需要同步 Linear 状态、Workpad、PR 链接和证据时，优先使用 `.claude/skills/harness-linear-loop/SKILL.md`。
